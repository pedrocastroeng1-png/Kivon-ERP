...
const putMatch = `// Update user profile/status
...
const putReplacement = `// Update user profile/status
apiRouter.put('/users/:id', requireAdmin, async (req, res) => {
  const { id } = req.params;
  const { fullName, profileCode, active, forcePasswordChange } = req.body;
...
code = code.replace(putMatch, putReplacement);

const oldPutQuery = `    const { data: oldUser } = await supabaseAdmin.from('users').select('*').eq('id', id).single();`;
code = code.replace(oldPutQuery, '');
